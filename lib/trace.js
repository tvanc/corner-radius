...
export function trace(el) {
  ...
  const pathCommandSets = draw(x, y, polygon)

  for (let i = 0; i < pathCommandSets.length; ++i) {
    const polygonCommands = pathCommandSets[i]
  ...
  for (let i = allPaths.length - 1; i >= pathCommandSets.length; --i) {
    allPaths[i].remove()
  }
