...
import { edgesMatch, getEdges, removeDuplicateEdges } from "../getPolygons"
...
test("removeDuplicateEdges()", function () {
  const rectangles = [
  ...
  const edges = getEdges(rectangles)
  const uniqueEdges = removeDuplicateEdges(edges)
  const edgesPerRectangle = 4
  const sharedEdges = rectangles.length - 1
  const sharedEdges = (rectangles.length - 1) * 2

  expect(uniqueEdges.length).toStrictEqual(
    edgesPerRectangle * rectangles.length - sharedEdges,
  )
})
